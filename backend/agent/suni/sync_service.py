...
from .config_manager import SuniConfigManager, SuniConfiguration
from .repository import SuniAgentRepository, SuniAgentRecord
...
class SuniSyncService:
    def __init__(self):
        self.config_manager = SuniConfigManager()
        self.repository = SuniAgentRepository()
    
    async def sync_all_agents(self, dry_run: bool = False) -> SyncResult:
        logger.info("🚀 Starting Suni agent metadata sync")
        
        try:
            current_config = self.config_manager.get_current_config()
            agents_needing_sync = await self.repository.find_suni_agents_needing_sync(
                current_config.version_tag
            )
            
            if not agents_needing_sync:
                logger.info("📋 All Suni agents already have current metadata")
                return SyncResult(
                    success=True,
                    synced_count=0,
    ...
    async def install_for_all_missing_users(self) -> SyncResult:
        logger.info("🚀 Installing Suni agents for users who don't have them")
        
        try:
            current_config = self.config_manager.get_current_config()
            all_accounts = await self.repository.get_all_personal_accounts()
            existing_agents = await self.repository.find_all_suni_agents()
            ...
            if not missing_accounts:
                return SyncResult(
                    success=True,
                    details=[{"message": "All users already have Suni agents"}]
                )
            
            logger.info(f"📦 Installing Suni for {len(missing_accounts)} users")
            ...
            for account_id in missing_accounts:
                try:
                    await self.repository.create_suni_agent_simple(
                        account_id,
                        current_config.version_tag
                    )
                    success_count += 1
                    logger.info(f"✅ Installed Suni for user {account_id}")
                except Exception as e:
                    failed_count += 1
    ...
    async def get_sync_status(self) -> Dict[str, Any]:
        try:
            ...
            return {
                ...
                "note": "System prompt & tools always current from SuniConfig"
            }
