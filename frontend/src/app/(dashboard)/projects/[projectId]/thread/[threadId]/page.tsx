...
export default function ThreadPage({
  ...
  useEffect(() => {
    if (projectName) {
      document.title = `${projectName} | Kortix Suni`;
      ...
      if (metaDescription) {
        metaDescription.setAttribute(
          'content',
          `${projectName} - Interactive agent conversation powered by Kortix Suni`,
        );
      }

      const ogTitle = document.querySelector('meta[property="og:title"]');
      if (ogTitle) {
        ogTitle.setAttribute('content', `${projectName} | Kortix Suni`);
      }
  ...
  useEffect(() => {
    if (initialLoadCompleted && subscriptionData && !hasCheckedUpgradeDialog.current) {
      ...
      const hasSeenUpgradeDialog = localStorage.getItem('suni_upgrade_dialog_displayed');
  ...
  const handleDismissUpgradeDialog = () => {
    ...
    localStorage.setItem('suni_upgrade_dialog_displayed', 'true');
  };
